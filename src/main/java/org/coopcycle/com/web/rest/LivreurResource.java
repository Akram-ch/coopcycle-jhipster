...
/**
 * REST controller for managing {@link org.coopcycle.com.domain.Livreur}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class LivreurResource {
    ...
    public ResponseEntity<Livreur> partialUpdateLivreur(
        ...
        Optional<Livreur> result = livreurRepository
            ...
            .map(existingLivreur -> {
                if (livreur.getLivId() != null) {
                    existingLivreur.setLivId(livreur.getLivId());
                }
                if (livreur.getName() != null) {
                    existingLivreur.setName(livreur.getName());
                }
