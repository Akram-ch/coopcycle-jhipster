...
import org.coopcycle.com.repository.PanierRepository;
...
/**
 * REST controller for managing {@link org.coopcycle.com.domain.Panier}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PanierResource {
     ...
     */
    @GetMapping("/paniers")
    public List<Panier> getAllPaniers(@RequestParam(required = false) String filter) {
        if ("client-is-null".equals(filter)) {
            log.debug("REST request to get all Paniers where client is null");
            return StreamSupport
                .stream(panierRepository.findAll().spliterator(), false)
                .filter(panier -> panier.getClient() == null)
